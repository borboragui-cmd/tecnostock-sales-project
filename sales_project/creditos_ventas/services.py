...
def procesar_tipo_pago(factura, tipo_pago, num_cuotas=None):
    """
    Aplica el tipo de pago elegido a una factura recién creada.
    ...
    if tipo_pago == 'CONTADO':
        ...
        factura.save(update_fields=['tipo_pago', 'saldo', 'estado'])
        return
    ...
    if tipo_pago == 'CREDITO':
        ...
        factura.save(update_fields=['tipo_pago', 'saldo', 'estado'])
        generar_cuotas(factura, num_cuotas)
        return
...
@transaction.atomic
def generar_cuotas(factura, num_cuotas):
    """
    ...
    mensuales a partir de la fecha de la factura.
    """
    if num_cuotas < 1:
        raise ValidationError('El número de cuotas debe ser al menos 1.')
    ...
    valor_base = (factura.total / num_cuotas).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    ...
    for i in range(1, num_cuotas + 1):
        if i < num_cuotas:
            ...
            valor_cuota = factura.total - acumulado  # última cuota absorbe el residuo

        cuota = CuotaVenta.objects.create(
            factura=factura,
            numero=i,
            fecha_vencimiento=factura.invoice_date.date() + relativedelta(months=i),
            valor=valor_cuota,
            saldo=valor_cuota,
        )
...
def registrar_pago(cuota_id, valor, observacion=''):
    ...
    siquiera fechas pasadas dentro del rango de la factura. select_for_update
    ...
    """
    cuota = CuotaVenta.objects.select_for_update().select_related('factura').get(pk=cuota_id)

    if cuota.factura.estado == 'PAGADA':
    ...
    factura.saldo = F('saldo') - capital_reducido
    factura.save(update_fields=['saldo'])
