...
def generar_pdf_plan_pagos(factura):
    """Cronograma completo de cuotas de una factura a crédito."""
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter, topMargin=2*cm, bottomMargin=2*cm)
    styles = getSampleStyleSheet()
    elementos = [
        Paragraph(f'Plan de Pagos - Factura {factura.numero}', styles['Title']),
        Spacer(1, 0.4*cm),
        Paragraph(f'Cliente: {factura.customer.full_name}', styles['Normal']),
        Paragraph(f'DNI/RUC: {factura.customer.dni}', styles['Normal']),
        Paragraph(f'Fecha de factura: {factura.invoice_date.strftime("%d/%m/%Y")}', styles['Normal']),
        Paragraph(f'Total: ${factura.total}', styles['Normal']),
        Paragraph(f'Saldo pendiente: ${factura.saldo}', styles['Normal']),
        Paragraph(f'Estado: {factura.estado}', styles['Normal']),
        Spacer(1, 0.6*cm),
    ]

    data = [['N°', 'Vencimiento', 'Valor', 'Saldo', 'Estado']]
    for cuota in factura.cuotas.order_by('numero'):
    ...
    response['Content-Disposition'] = f'inline; filename="plan_pagos_{factura.numero}.pdf"'
    return response
...
def generar_pdf_comprobante_lote(pagos):
    ...
    data = [['Factura', 'Cliente', 'Cuota N°', 'Fecha', 'Valor']]
    comandos = _comandos_tabla_estandar()

    for pago in pagos:
        cuota = pago.cuota
        data.append([
            cuota.factura.numero, cuota.factura.customer.full_name,
